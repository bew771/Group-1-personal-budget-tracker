...
$defaultSources = ['Salary', 'Bonus', 'Gift', 'Rent', 'Other'];
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_tx') {
    ...
    $category_id = null;
    // Handle default source option values like "source:Salary" or numeric category IDs
    if ($rawCategory !== '') {
    ...
    if ($amount <= 0) { $error = 'Amount must be greater than zero.'; }
        ...
        $stmt->execute([$_SESSION['user_id'], $category_id, $amount, 'income', $note, $date]);
        header('Location: income.php'); exit;
    }
}
$cats = $pdo->prepare('SELECT id, name FROM categories WHERE user_id = ? AND type = ? ORDER BY name');
$cats->execute([$_SESSION['user_id'], 'income']);
$incomeCats = $cats->fetchAll();
...
$txStmt->execute([$_SESSION['user_id'], 'income']);
...
<div class="container mt-4">
  ...
  <form method="post" class="row g-3 mb-4">
    ...
    <div class="col-md-2"><button class="btn btn-success">Add Income</button></div>
  </form>
...
<script>
  // Show/hide custom source box when Other is selected
  (function () {
